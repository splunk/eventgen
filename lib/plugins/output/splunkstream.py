    ...
    def __init__(self, sample):
        ...
        if self._sample.sessionKey == None:
            try:
                ...
                logger.error("Error getting session key for non-SPLUNK_EMBEDDED for sample '%s'.  Credentials are missing or wrong" % self._sample.name)
                raise IOError("Error getting session key for non-SPLUNK_EMBEDDED for sample '%s'.  Credentials are missing or wrong" % self._sample.name)
