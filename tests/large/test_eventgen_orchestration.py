...
class TestEventgenOrchestration(object):
    ...
    def test_controller_root(self):
        r = requests.get(
            "http://127.0.0.1:{}/".format(self.controller_eventgen_webport)
        )
        assert r.status_code == 200
        assert b"running_eventgen_controller" in r.content

    def test_controller_index(self):
        r = requests.get(
            "http://127.0.0.1:{}/index".format(self.controller_eventgen_webport)
        )
        assert r.status_code == 200
        assert b"Eventgen Controller" in r.content
        assert b"Host: " in r.content
        assert b"You are running Eventgen Controller" in r.content
    ...
    def test_controller_status(self):
        ...
        while not output and current_retry <= max_retry:
            response = requests.get(
                "http://127.0.0.1:{}/status".format(self.controller_eventgen_webport),
                timeout=10,
            )
    ...
    def test_controller_conf(self):
        r = requests.post(
            "http://127.0.0.1:{}/conf".format(self.controller_eventgen_webport),
            json=self.test_json,
        )
        assert r.status_code == 200
        assert b"windbag" in r.content

    def test_controller_start(self):
        r = requests.post(
            "http://127.0.0.1:{}/start".format(self.controller_eventgen_webport)
        )
        assert r.status_code == 200
        assert b"Eventgen has successfully started" in r.content

    def test_controller_start_with_target(self):
        r = requests.post(
            "http://127.0.0.1:{}/start/{}".format(
                self.controller_eventgen_webport,
                TestEventgenOrchestration.server_id[:12],
            )
        )
        assert r.status_code == 200
        assert b"Eventgen already started" in r.content

    def test_controller_restart(self):
        r = requests.post(
            "http://127.0.0.1:{}/restart".format(self.controller_eventgen_webport)
        )
        assert r.status_code == 200
        assert b"Eventgen is restarting" in r.content

    def test_controller_restart_with_target(self):
        r = requests.post(
            "http://127.0.0.1:{}/restart/{}".format(
                self.controller_eventgen_webport,
                TestEventgenOrchestration.server_id[:12],
            )
        )
    ...
    def test_controller_bundle_invalid_request(self):
        ...
        assert b"Internal Error Occurred" in r.content
    ...
    def test_controller_set_volume_with_volume_and_target(self):
        r = requests.post(
            "http://127.0.0.1:{}/volume/{}".format(
                self.controller_eventgen_webport,
                TestEventgenOrchestration.server_id[:12],
            ),
            json={"perDayVolume": 20},
        )
        assert r.status_code == 200
        output = json.loads(r.content)
    ...
    def test_controller_stop(self):
        r = requests.post(
            "http://127.0.0.1:{}/stop".format(self.controller_eventgen_webport)
        )
        assert r.status_code == 200
        assert r.status_code == 200
        assert b"Eventgen is stopping" in r.content

    def test_controller_stop_with_target(self):
        r = requests.post(
            "http://127.0.0.1:{}/stop/{}".format(
                self.controller_eventgen_webport,
                TestEventgenOrchestration.server_id[:12],
            )
        )
    ...
    def test_server_reset(self):
        r = requests.post(
            "http://127.0.0.1:{}/reset".format(self.server_eventgen_webport)
        )
        assert r.status_code == 200
    ...
    def test_server_root(self):
        ...
        assert r.status_code == 200
        assert b"running_eventgen_server" in r.content

    def test_server_index(self):
        r = requests.get(
            "http://127.0.0.1:{}/index".format(self.server_eventgen_webport)
        )
        assert r.status_code == 200
        assert b"Host: " in r.content
        assert b"Eventgen Status" in r.content
        assert b"Eventgen Config file exists" in r.content
        assert b"Eventgen Config file path" in r.content
        assert b"Total volume:" in r.content
        assert b"Worker Queue Status" in r.content
        assert b"Sample Queue Status" in r.content
        assert b"Output Queue Status" in r.content

    def test_server_status(self):
        r = requests.get(
            "http://127.0.0.1:{}/status".format(self.server_eventgen_webport)
        )
        assert r.status_code == 200
        output = json.loads(r.content)
        assert output
    ...
    def test_server_get_and_set_conf(self):
        r = requests.get(
            "http://127.0.0.1:{}/conf".format(self.server_eventgen_webport)
        )
        assert r.status_code == 200
        assert json.loads(r.content)
        config_json = {"windbag": {"end": "10"}}
        r = requests.post(
            "http://127.0.0.1:{}/conf".format(self.server_eventgen_webport),
            json=config_json,
        )
    ...
    def test_server_start(self):
        r = requests.post(
            "http://127.0.0.1:{}/start".format(self.server_eventgen_webport), timeout=5
        )
        assert r.status_code == 200
        assert b"Eventgen has successfully started" in r.content
    ...
    def test_server_restart(self):
        r = requests.post(
            "http://127.0.0.1:{}/restart".format(self.server_eventgen_webport)
        )
        assert r.status_code == 200
        assert b"Eventgen has successfully restarted" in r.content

    def test_server_stop(self):
        r = requests.post(
            "http://127.0.0.1:{}/stop".format(self.server_eventgen_webport)
        )
        assert r.status_code == 200
        assert b"Eventgen is stopped" in r.content

    def test_server_bundle(self):
        r = requests.post(
            "http://127.0.0.1:{}/bundle".format(self.server_eventgen_webport)
        )
        assert r.status_code == 500
        assert b"Internal Error Occurred" in r.content
    ...
    def test_server_get_and_set_volume(self):
        ...
        assert json.loads(r.content)
        r = requests.post(
            "http://127.0.0.1:{}/volume".format(self.server_eventgen_webport),
            json={"perDayVolume": 10},
        )
        assert r.status_code == 200
        assert json.loads(r.content)
