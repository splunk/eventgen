...
@mark.parametrize(("conf_filename", "execution_timeout", "perdayvolume"),
                  [
                      ("eventgen_perdayvolume.conf", 300, 1),
                      ("eventgen_perdayvolume_small_token.conf", 300, 1),
                      ("eventgen_perdayvolume_large_token.conf", 300, 1)
                  ])
def test_perdayvolume(eventgen_test_helper, conf_filename, execution_timeout, perdayvolume):
    ...
    assert 0.98 < (calculate_perdayvolume(events, execution_timeout) / perdayvolume) < 1.02
