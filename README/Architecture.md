...
* Creating timers for each sample
    ...
    * For samples which are not queueable (meaning we can only run one simultaneous threa of the generator), the thread runs the generator plugin single threaded and handles sleeping the proper amount to manage intervals.
* Managing workers
