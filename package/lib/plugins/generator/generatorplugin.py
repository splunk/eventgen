...
class GeneratorPlugin:
    ...
    def _openSampleFile(self):
        self.logger.debugv("Opening sample '%s' in app '%s'" % (self._sample.name, self._sample.app))
        self._sampleFH = open(self._sample.filePath, 'rU')

    def _closeSampleFile(self):
        self.logger.debugv("Closing sample '%s' in app '%s'" % (self._sample.name, self._sample.app))
        ...
        if s.sampletype == 'raw':
            ...
            if s.sampleDict == None:
                ...
                if s.breaker == c.breaker:
                    self.logger.debugv("Reading raw sample '%s' in app '%s'" % (s.name, s.app))
            ...
            if s.sampleDict == None:
                self._openSampleFile()
                self.logger.debugv("Reading csv sample '%s' in app '%s'" % (s.name, s.app))
                self.sampleDict = [ ]
                ...
                self.logger.debug('Finished creating sampleDict & sampleLines.  Len sampleDict: %d' % (len(self.sampleDict)))
            else:
                self.sampleDict = s.sampleDict
                self.sampleLines = s.sampleLines
