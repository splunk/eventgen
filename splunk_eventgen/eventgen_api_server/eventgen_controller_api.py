...
You are running Eventgen Controller.\n'''
        ...
        def http_status(target):
            try:
                ...
                return Response(json.dumps(gather_response('status', message_uuid=message_uuid, response_number_target=0 if target == 'all' else 1)), mimetype='application/json', status=200)
        ...
        def http_conf(target):
            try:
                ...
                message_uuid = publish_message('conf', request.method, body=body, target=target)
                return Response(json.dumps(gather_response('conf', message_uuid=message_uuid, response_number_target=0 if target == 'all' else 1)), mimetype='application/json', status=200)
            except Exception as e:
                self.logger.error(e)
                return Response(INTERNAL_ERROR_RESPONSE, mimetype='application/json', status=500)
        ...
        def http_bundle(target):
            try:
                message_uuid = publish_message('bundle', request.method, body=request.get_json(force=True), target=target)
                return Response(json.dumps(gather_response('bundle', message_uuid=message_uuid, response_number_target=0 if target == 'all' else 1)), mimetype='application/json', status=200)
            except Exception as e:
                self.logger.error(e)
                return Response(INTERNAL_ERROR_RESPONSE, mimetype='application/json', status=500)
        ...
        def http_volume(target):
            try:
                body = None if request.method == 'GET' else request.get_json(force=True)
                message_uuid = publish_message('volume', request.method, body=body, target=target)
                return Response(json.dumps(gather_response('volume', message_uuid=message_uuid, response_number_target=0 if target == 'all' else 1)), mimetype='application/json', status=200)
            except Exception as e:
                self.logger.error(e)
                return Response(INTERNAL_ERROR_RESPONSE, mimetype='application/json', status=500)
        ...
        def http_start(target):
            try:
                ...
                return Response(json.dumps(gather_response('start', message_uuid=message_uuid, response_number_target=0 if target == 'all' else 1)), mimetype='application/json', status=200)
            except Exception as e:
                self.logger.error(e)
                return Response(INTERNAL_ERROR_RESPONSE, mimetype='application/json', status=500)
        ...
        def http_stop(target):
            try:
                ...
                return Response(json.dumps(gather_response('stop', message_uuid=message_uuid, response_number_target=0 if target == 'all' else 1)), mimetype='application/json', status=200)
            except Exception as e:
                self.logger.error(e)
                return Response(INTERNAL_ERROR_RESPONSE, mimetype='application/json', status=500)
        ...
        def http_restart(target):
            try:
                ...
                return Response(json.dumps(gather_response('restart', message_uuid=message_uuid, response_number_target=0 if target == 'all' else 1)), mimetype='application/json', status=200)
            except Exception as e:
                self.logger.error(e)
                return Response(INTERNAL_ERROR_RESPONSE, mimetype='application/json', status=500)
