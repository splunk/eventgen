...
class EventGenerator(object):
    ...
    def _create_generator_pool(self, workercount=20):
        ...
        self.gc_dict = {s.name: multiprocessing.Value('i', 0) for s in self.config.samples if hasattr(s, 'disableGlobalCount') and s.disableGlobalCount == 'false'}
    ...
    def _generator_do_work(self, work_queue, logging_queue, output_counter=None, **kwargs):
        while not self.stop_request.isSet():
            try:
                item = work_queue.get(timeout=10)
                gc_obj = kwargs.get("gc_dict", {}).get(item._sample.name)
                setattr(item, "gc_obj", gc_obj)
                startTime = time.time()
    ...
    @staticmethod
    def _proc_worker_do_work(work_queue, logging_queue, config, disable_logging, **kwargs):
        genconfig = config
    ...
    def start(self, join_after_start=True):
        ...
        if hasattr(self.config, "scsAccessToken"):
            for s in self.config.samples:
                setattr(s, "scsAccessToken", self.config.scsAccessToken)
