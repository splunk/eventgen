...
class AwsS3OutputPlugin(OutputPlugin):
    ...
    def __init__(self, sample, config=None):
        ...
        adapter = EventgenAdapter(logger, {'module': 'AwsS3OutputPlugin',
                                           'sample': sample.name})
    ...
    def _transmitEvents(self, payloadstring):
        logger.debug("Transmission called with payloadstring event number: %d "
                     % len(payloadstring))
        ...
        logger.debugv("Uploading %d events into s3 key: %s " %
        ...
        try:
            response = self._client.put_object(Bucket=self.awsS3bucketname,
                                               Key=s3keyname,
                                               Body=records)
            logger.debugv("response = %s" % response)
        except Exception as e:
            logger.error("Failed for exception: %s" % e)
            logger.debugv("Failed sending events to payload: %s" %
                          (payloadstring))
            raise e
