...
def timeParser(ts="now", timezone=datetime.timedelta(days=1), now=None, utcnow=None):
    if ts == "now":
        ...
        if ts[:1] == "+" or ts[:1] == "-":
            ...
            unitsre = (
                "(seconds|second|secs|sec|minutes|minute|min|hours|hour|hrs|hr|days|day|weeks|week|w[0-6]|"
                + "months|month|mon|quarters|quarter|qtrs|qtr|years|year|yrs|yr|s|h|m|d|w|y|w|q)"
            )
            reltimere = (
                r"(?i)(?P<plusminus>[+-]*)(?P<num>\d{1,})(?P<unit>"
                + unitsre
                + r"{1})(([\@](?P<snapunit>"
                + unitsre
                + r"{1})((?P<snapplusminus>[+-])(?P<snaprelnum>\d+)(?P<snaprelunit>"
                + unitsre
                + "{1}))*)*)"
            )
            ...
            if (
                resultsdict["plusminus"] is not None
                and resultsdict["num"] is not None
                and resultsdict["unit"] is not None
            ):
                ret = timeParserTimeMath(
                ...
                if resultsdict["snapunit"] is not None:
                    if resultsdict["snapunit"] in (
                        ...
                        ret = datetime.datetime(
                            ret.year, ret.month, ret.day, ret.hour, ret.minute, 0, 0
                        )
                    elif resultsdict["snapunit"] in ("h", "hr", "hrs", "hour", "hours"):
                        ret = datetime.datetime(
                            ret.year, ret.month, ret.day, ret.hour, 0, 0, 0
                        )
                    elif resultsdict["snapunit"] in ("d", "day", "days"):
                        ret = datetime.datetime(
                            ret.year, ret.month, ret.day, 0, 0, 0, 0
                        )
                    elif re.match(
                        "w[0-6]", resultsdict["snapunit"]
                    ) is not None or resultsdict["snapunit"] in ("w", "week", "weeks"):
                        ...
                        if weekdaynum <= retweekday:
                            ret = ret + datetime.timedelta(
                                days=(weekdaynum - retweekday)
                            )
                            ret = datetime.datetime(
                                ret.year, ret.month, ret.day, 0, 0, 0, 0
                            )
                    ...
                    # Normalize out all year/quarter/months to months and do the math on that
                        ...
                        ret = datetime.datetime(
                            ret.year,
                            int(math.floor(ret.month / 3.3 + 1) * 3),
                            1,
                            0,
                            0,
                            0,
                    ...
                    if (
                        resultsdict["snapplusminus"] is not None
                        and resultsdict["snaprelnum"] is not None
                        and resultsdict["snaprelunit"] is not None
                    ):
                        ret = timeParserTimeMath(
                            resultsdict["snapplusminus"],
                            resultsdict["snaprelnum"],
                            resultsdict["snaprelunit"],
                            ret,
                        )
                return ret
...
def timeParserTimeMath(plusminus, num, unit, ret):
    try:
        ...
        if unit in ("s", "sec", "secs", "second", "seconds"):
            ...
            logger.error(
                "Day of week is only available in snap-to.  Time string: %s" % td
            )
            return False
        # Normalize out all year/quarter/months to months and do the math on that
        elif (
...
def timeDelta2secs(timeDiff):
    deltaSecs = (
        timeDiff.microseconds + (timeDiff.seconds + timeDiff.days * 24 * 3600) * 10 ** 6
    ) / 10 ** 6
